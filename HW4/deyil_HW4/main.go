...
func (t *Turmite) Step(field Field) error {
	...
	if act, ok := t.rules[sig]; ok {
		t.state = act.state
		field[t.y][t.x] = act.color
		if act.turn == BackwardDir {
			t.currentDir = t.currentDir.Left().Left()
		} else if act.turn == LeftDir {
			t.currentDir = t.currentDir.Left()
		} else if act.turn == RightDir {
	...
	if t.currentDir == NorthDir {
		t.y--
	} else if t.currentDir == Direction(NorthDir).Left() {
		t.x--
	} else if t.currentDir == Direction(NorthDir).Left().Left() {
		t.y++
	} else if t.currentDir == Direction(NorthDir).Right() {
		t.x++
	}
...
func main() {
	...
	flag.IntVar(&fieldSize, "s", 500, "Size of the field")
	flag.IntVar(&iters, "steps", 30000, "Number of steps")
